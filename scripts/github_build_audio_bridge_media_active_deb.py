#!/usr/bin/env python3
"""Build the experimental iOS-VCAM AudioBridgeMediaActive .deb via GitHub Actions.

This Windows-friendly wrapper uses:

    .github/workflows/build-audio-bridge-media-active-tweak.yml
...
DEFAULT_WORKFLOW = "build-audio-bridge-media-active-tweak.yml"
DEFAULT_ARTIFACT = "iosvcam-audio-bridge-media-active-rootless-deb"
DEFAULT_DOWNLOAD_DIR = Path("ios/audio_bridge_media_active_tweak/packages")
COMMIT_PATHS = [
    ".github/workflows/build-audio-bridge-media-active-tweak.yml",
    "ios/audio_bridge_media_active_tweak/Makefile",
    "ios/audio_bridge_media_active_tweak/Tweak.x",
    "ios/audio_bridge_media_active_tweak/control",
    "ios/audio_bridge_media_active_tweak/README.md",
    ...
    "scripts/github_build_audio_bridge_media_active_deb.py",
...
def commit_changes(paths: list[str]) -> None:
    step("Committing media-active build inputs")
    ...
    message = (
        "Add audio bridge media-active build\n\n"
        "Add an experimental mediaserverd active AudioBridge package for manual RootHide testing.\n\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build iOS-VCAM AudioBridgeMediaActive .deb via GitHub Actions")
    ...
    parser.add_argument("--commit-changes", action="store_true", help="Commit media-active build inputs before pushing")
