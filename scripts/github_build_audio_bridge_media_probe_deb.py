#!/usr/bin/env python3
"""Build the passive iOS-VCAM AudioBridgeMediaProbe .deb via GitHub Actions.

This Windows-friendly wrapper uses:

    .github/workflows/build-audio-bridge-media-probe-tweak.yml
...
DEFAULT_WORKFLOW = "build-audio-bridge-media-probe-tweak.yml"
DEFAULT_ARTIFACT = "iosvcam-audio-bridge-media-probe-rootless-deb"
DEFAULT_DOWNLOAD_DIR = Path("ios/audio_bridge_media_probe_tweak/packages")
COMMIT_PATHS = [
    ".github/workflows/build-audio-bridge-media-probe-tweak.yml",
    ".claude/commands/build-audio-bridge-media-probe-deb.md",
    "ios/audio_bridge_media_probe_tweak/Makefile",
    "ios/audio_bridge_media_probe_tweak/Tweak.x",
    "ios/audio_bridge_media_probe_tweak/control",
    "ios/audio_bridge_media_probe_tweak/README.md",
    ...
    "scripts/github_build_audio_bridge_media_probe_deb.py",
...
def commit_changes(paths: list[str]) -> None:
    step("Committing media probe build inputs")
    ...
    message = (
        "Add audio bridge media probe build\n\n"
        "Add a passive media-layer probe package for RootHide-independent AudioBridge testing.\n\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build iOS-VCAM AudioBridgeMediaProbe .deb via GitHub Actions")
    ...
    parser.add_argument("--commit-changes", action="store_true", help="Commit media-probe build inputs before pushing")
