#!/usr/bin/env python3
"""Build the iOS-VCAM AudioBridge system-hook .deb via GitHub Actions.

This Windows-friendly wrapper uses:

    .github/workflows/build-audio-bridge-system-hook-tweak.yml

It builds and downloads an artifact only. It does not install anything on the
iPhone or restart mediaserverd.
"""
...
DEFAULT_WORKFLOW = "build-audio-bridge-system-hook-tweak.yml"
DEFAULT_ARTIFACT = "iosvcam-audio-bridge-system-hook-rootless-deb"
DEFAULT_DOWNLOAD_DIR = Path("ios/audio_bridge_system_tweak/packages")
COMMIT_PATHS = [
    ".github/workflows/build-audio-bridge-system-hook-tweak.yml",
    ...
    "ios/audio_bridge_system_tweak/README.md",
...
def commit_changes(paths: list[str]) -> None:
    step("Committing AudioBridge system-hook build inputs")
    ...
    message = (
        "Add audio bridge system hook build\n\n"
        "Add the passive mediaserverd system-hook package inputs for AudioBridge System v0.1.\n\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build iOS-VCAM AudioBridge system-hook .deb via GitHub Actions")
    ...
    parser.add_argument("--commit-changes", action="store_true", help="Commit system-hook build inputs before pushing")
