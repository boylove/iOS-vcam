...
Audio (OBS microphone replacement) is provided by the OpenVCam tweak itself (`com.iosvcam.opencam`): OBS audio is demuxed from the same RTMP stream and replaces the microphone in `mediaserverd`, gated by the floating panel's 替换音频 switch. No separate PC-side bridge package or second tunnel is required.
