// ---------------------------------------------------------------------------
// OpenVCam — audio subsystem (merged from ios/audio_bridge_safe_tweak).
//
// This is the app-level SAFE audio bridge, injected ONLY into TikTok
// (com.zhiliaoapp.musically / process "TikTok"): its %ctor returns early for
// every other process. It NEVER runs in mediaserverd and NEVER touches the
// stock Camera app — that is the whole point. The freeze the user hit when the
// stock Camera switches photo->video (which then also kills the OBS video
// overlay) comes from the SYSTEM-level audio hook (iOSVCAMAudioBridgeSystemHook,
// which does hook mediaserverd) — NOT this file. The combined package Conflicts
// that system package so the two can't coexist. See EXECUTION-PLAN §4.5 and
// memory unsafe-ios-audio-bridge-tweak / safe-audiobridge-tiktok-035.
//
// Kept behaviourally identical to the verified com.iosvcam.audiobridge.safe
// logic (same CFPreferences domain, so the user's existing audio settings and
// disable flag keep working after the merge).
// ---------------------------------------------------------------------------
