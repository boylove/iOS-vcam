...
**❌ 0.5.0 装机实测仍卡(2026-07-08,决定性 syslog 证据)**:删旋转后照片模式**依旧"动一下就冻结在 OBS 帧"**。抓 `idevicesyslog` 坐实真凶:内核每 ~61s 刷 `kernel(AppleM2ScalerCSC): WARNING: AppleM2ScalerCSCDriver: IOFence is hung`——**VTPixelTransfer 驱动的硬件缩放/色彩转换器(AppleM2ScalerCSC)的 GPU IOFence 挂死**。mediaserverd **未被杀**(health `emits` 持续从 40800 爬到 60000),故是"冻结在最后一帧 OBS"而非黑屏;`replaced` 在挂死瞬间冻结在 3473。→ 单次直传本身仍挂缩放器 fence,**旋转不是(唯一)根因**。

**✅ 0.5.1 修复(对齐原版 GPU 同步纪律)**:唯一与原版不同的动作是 0.5.0 保留的 **transfer 后 `CVPixelBufferLockBaseAddress` fence flush**——原版反汇编证明**其 transfer 前后从不锁 CVPixelBuffer**(记忆 `openvcam-original-gpu-sync-model` 点 1),而 Lock 正是"CPU 等 GPU fence"的操作,在照片模式(自带 BWStillImageScalerNode 也在抢同一 AppleM2ScalerCSC)里闭合 fence 环。**修复**:①`VCAM_GPU_FENCE_FLUSH` 默认改 0(去掉 flush,不锁);②给 transfer session 显式设 `EnableGPUAcceleratedTransfer=true`(原版有,analysis §3.5)。**待装机验收**(切照片模式 + 快门无 `AppleM2ScalerCSC IOFence is hung`、预览不冻结、存图即 OBS)。
